...
import { GameIconAnimationsIndex } from "../resources";
...
function ResolveAnimationName(iconName: string, stateName: string, currentDirection: number | undefined = undefined): string | undefined {

  const fieldName = iconName as keyof typeof GameIconAnimationsIndex;
  const animations = GameIconAnimationsIndex[fieldName];

  if (!animations.find(v => v.startsWith(stateName)))
    stateName = animations[0].substr(0, animations[0].lastIndexOf('_'));

  const stateAnimations = [...GameIconAnimationsIndex[fieldName].filter(v => v.startsWith(stateName))];
