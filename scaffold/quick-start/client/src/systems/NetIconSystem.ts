...
import { GameIcons } from "../resources";
...
export class NetIconSystem extends System<NetConfigureIconComponent> {
  ...
  private resolveIconAnimation(e: Entity): string | undefined {
    ...
    return ResolveAnimationName(icon.icon!.icon, icon.icon!.state, dir);
  }
  ...
  public updateGraphic(entity: Actor, config: NetConfigureIconComponent) {
    ...
    const fieldName = icon as keyof typeof GameIcons;

    if (!GameIcons.hasOwnProperty(fieldName))
      return;
    ...
    const gfx = GameIcons[fieldName].getAnimation(animation)! as ex.Animation;
