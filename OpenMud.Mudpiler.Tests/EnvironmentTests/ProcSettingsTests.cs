...
";
            ...
            Assert.IsTrue(attributes.Count() == 0 || attributes.Single().Background == false);
        }
        [Test]
        public void TestProcHiddenAttribute()
        {
            var dmlCode =
                @"
/mob/test
...
";
            ...
            var proc = atm.GetProc("testa_a");
            var attributes = proc.Attributes().Where(x => x is Hidden).Cast<Hidden>();
            Assert.IsTrue(attributes.Single().IsHidden == true);

            proc = atm.GetProc("testa_b");
            attributes = proc.Attributes().Where(x => x is Hidden).Cast<Hidden>();
            Assert.IsTrue(attributes.Count() == 0 || attributes.Single().IsHidden == true);
        }
    }
}
